...
namespace Flowthru.Spark
{
      ...
      IJvmObjectReferenceProvider
  {
    ...
    {
      var localDir = (string)
        conf.Reference.Jvm.CallStaticJavaMethod("org.apache.spark.util.Utils", "getLocalDir", conf);
    ...
    {
      var javaSparkContext = (JvmObjectReference)
        sc.Reference.Jvm.CallStaticJavaMethod(
    ...
    )
    {
      ...
      // Spark versions.
      bool encryptionEnabled = bool.Parse(sc.GetConf().Get("spark.io.encryption.enabled", "false"));
      ...
      {
        _pythonBroadcast = (JvmObjectReference)
          javaSparkContext.Jvm.CallStaticJavaMethod(
    ...
    private void Dump(object value, Stream stream) => BinarySerDe.Serialize(stream, value);
