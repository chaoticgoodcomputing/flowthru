  ...
  internal sealed class CallbackServer
  {
    ...
    internal void Run(ISocketWrapper listener)
    {
      ...
      try
      {
        ...
        _jvmCallbackClient = (JvmObjectReference)
        ...
        new Thread(() => StartServer(_listener)) { IsBackground = true }.Start();
      }
      catch (Exception e)
      {
    ...
    private void StartServer(ISocketWrapper listener)
    {
      try
      {
        ...
        while (_isRunning)
        {
          ...
          var connection = new CallbackConnection(connectionId, socket, _callbackHandlers);
    ...
    private void RunWorkerThread()
    {
      try
      {
        while (_isRunning)
        {
          if (_waitingConnections.TryTake(out CallbackConnection connection, Timeout.Infinite))
          {
