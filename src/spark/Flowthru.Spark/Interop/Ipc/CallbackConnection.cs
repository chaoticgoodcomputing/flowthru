...
namespace Flowthru.Spark.Interop.Ipc
{
  ...
  {
    private static readonly ILoggerService s_logger = LoggerServiceFactory.GetLogger(
      typeof(CallbackConnection)
    );
    ...
    )
    {
      ...
      s_logger.LogInfo($"[{ConnectionId}] Connected with RemoteEndPoint: {socket.RemoteEndPoint}");
    }
    ...
    internal void Run(CancellationToken token)
    {
      ...
      try
      {
        while (_isRunning)
        {
          ConnectionStatus connectionStatus = ProcessStream(
            inputStream,
            outputStream,
          ...
          if (connectionStatus == ConnectionStatus.OK)
          {
            ...
            if (!readComplete)
            {
              ...
              s_logger.LogInfo($"[{ConnectionId}] Sleep 500 millisecond to close socket.");
              Thread.Sleep(500);
            }
          }
          ...
          {
            _isRunning = false;
            s_logger.LogInfo($"[{ConnectionId}] Request to close connection received.");
          }
          else
          {
            _isRunning = false;
      ...
      finally
      {
        ...
        s_logger.LogInfo($"[{ConnectionId}] Finished running {_numCallbacksRun} callback(s).");
      }
    }
    ...
    )
    {
      ...
      try
      {
        ...
        if (!_callbackHandlers.TryGetValue(callbackId, out ICallbackHandler callbackHandler))
        {
        ...
        using var callbackDataStream = new MemoryStream(
          SerDe.ReadBytes(inputStream, SerDe.ReadInt32(inputStream))
        );
        ...
        else
        {
          ...
          s_logger.LogWarn($"[{ConnectionId}] Unexpected end of stream: {endOfStream}.");
