...
namespace Flowthru.Spark.Interop
{
  /// <summary>
  ...
  public static class SparkEnvironment
  {
    ...
    private static Version GetSparkVersion()
    {
      ...
      string sparkVersionOverride = Environment.GetEnvironmentVariable("SPARK_VERSION_OVERRIDE");
      if (!string.IsNullOrEmpty(sparkVersionOverride))
      {
        s_logger.LogInfo(
          $"Overriding the Spark version from '{sparkVersion}' " + $"to '{sparkVersionOverride}'."
        );
    ...
    {
      get { return s_sparkVersion.Value; }
    }

    private static IJvmBridgeFactory s_jvmBridgeFactory;
    internal static IJvmBridgeFactory JvmBridgeFactory
    {
      get { return s_jvmBridgeFactory ??= new JvmBridgeFactory(); }
      set { s_jvmBridgeFactory = value; }
    }
    ...
    public static IJvmBridge JvmBridge
    {
      get
      {
        return s_jvmBridge ??= JvmBridgeFactory.Create(ConfigurationService.GetBackendPortNumber());
      }
      set { s_jvmBridge = value; }
    }

    private static IConfigurationService s_configurationService;
    internal static IConfigurationService ConfigurationService
    {
      get { return s_configurationService ??= new ConfigurationService(); }
      set { s_configurationService = value; }
    }

    private static CallbackServer s_callbackServer;
    internal static CallbackServer CallbackServer
    {
      get { return s_callbackServer ??= new CallbackServer(JvmBridge); }
    }
  }
}
