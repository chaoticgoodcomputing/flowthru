...
namespace Flowthru.Spark.Sql.Types
{
  ...
  public sealed class ArrayType : DataType
  {
    ...
    /// </summary>
    public override string SimpleString => string.Format("array<{0}>", ElementType.SimpleString);
  ...
  public sealed class MapType : DataType
  {
    ...
    internal override object FromInternal(object obj)
    {
      ...
      {
        convertedHashtable[KeyType.FromInternal(entry.Key)] = ValueType.FromInternal(entry.Value);
      }
