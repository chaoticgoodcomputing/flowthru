...
namespace Flowthru.Spark.Sql.Types
{
  ...
  public abstract class DataType
  {
    ...
    private static readonly Lazy<string[]> s_simpleTypeNormalizedNames = new Lazy<string[]>(
      () => s_simpleTypes.Select(t => NormalizeTypeName(t.Name)).ToArray()
    );

    private static readonly Lazy<string[]> s_complexTypeNormalizedNames = new Lazy<string[]>(
    ...
    public string Json
    {
      get
      {
        object jObject = (JsonValue is JObject) ? ((JObject)JsonValue).SortProperties() : JsonValue;
