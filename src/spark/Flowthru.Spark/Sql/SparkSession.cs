...
namespace Flowthru.Spark.Sql
{
  ...
  public sealed class SparkSession : IDisposable, IJvmObjectReferenceProvider
  {
    ...
    private static readonly string s_sparkSessionClassName = "org.apache.spark.sql.SparkSession";
    ...
    internal SparkSession(JvmObjectReference jvmObject)
    {
      ...
      _sparkContext = new Lazy<SparkContext>(
        () => new SparkContext((JvmObjectReference)Reference.Invoke("sparkContext"))
      );
    ...
    public static SparkSession Active() =>
      new SparkSession(
        (JvmObjectReference)
          SparkEnvironment.JvmBridge.CallStaticJavaMethod(s_sparkSessionClassName, "active")
      );
    ...
    public RuntimeConfig Conf() => new RuntimeConfig((JvmObjectReference)Reference.Invoke("conf"));
    ...
    public DataFrame CreateDataFrame(IEnumerable<GenericRow> data, StructType schema) =>
      new DataFrame(
        (JvmObjectReference)
          Reference.Invoke("createDataFrame", data, DataType.FromJson(Reference.Jvm, schema.Json))
      );
    ...
    public DataFrame ExecuteCommand(
      ...
      new DataFrame(
        (JvmObjectReference)Reference.Invoke("executeCommand", runner, command, options)
      );
    ...
    public DataFrame Range(long start, long end, long step, int numPartitions) =>
      new DataFrame((JvmObjectReference)Reference.Invoke("range", start, end, step, numPartitions));
