...
namespace Flowthru.Spark.Sql.Streaming
{
  ...
  public sealed class DataStreamWriter : IJvmObjectReferenceProvider
  {
    ...
    /// <returns>This DataStreamWriter object</returns>
    public DataStreamWriter OutputMode(OutputMode outputMode) => OutputMode(outputMode.ToString());
    ...
    {
      RDD.WorkerFunction.ExecuteDelegate wrapper = new ForeachWriterWrapperUdfWrapper(
        new ForeachWriterWrapper(writer).Process
      ).Execute;
