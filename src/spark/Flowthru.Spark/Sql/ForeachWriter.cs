...
namespace Flowthru.Spark.Sql
{
  ...
  internal class ForeachWriterWrapper
  {
    ...
    internal ForeachWriterWrapper(IForeachWriter foreachWriter) => _foreachWriter = foreachWriter;
    ...
    internal IEnumerable<object> Process(int partitionId, IEnumerable<Row> rows)
    {
      if (
        !TaskContextHolder
          .Get()
          .LocalProperties.TryGetValue("streaming.sql.batchId", out string epochIdStr)
        || !long.TryParse(epochIdStr, out long epochId)
      )
      {
        throw new Exception(
  ...
  internal class ForeachWriterWrapperUdfWrapper
  {
    ...
    internal ForeachWriterWrapperUdfWrapper(Func<int, IEnumerable<Row>, IEnumerable<object>> func)
    {
      _func = func;
    }
