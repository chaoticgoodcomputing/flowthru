...
namespace Flowthru.Spark.Sql
{
  ...
  public sealed class Row
  {
    ...
    internal Row(object[] values, StructType schema)
    {
      ...
      {
        throw new Exception($"Column count mismatches: data:{Size()}, schema:{schemaColumnCount}");
      }
    ...
    /// <returns>A column value</returns>
    public object Get(string columnName) => Get(Schema.Fields.FindIndex(f => f.Name == columnName));
