...
namespace Flowthru.Spark.ML.Feature
{
  ...
  /// Model Type.
  /// </typeparam>
  public abstract class JavaModel<M> : JavaTransformer, IModel<M>
    where M : JavaModel<M>
  {
    internal JavaModel(string className)
      : base(className) { }

    internal JavaModel(string className, string uid)
      : base(className, uid) { }

    internal JavaModel(JvmObjectReference jvmObject)
      : base(jvmObject) { }
    ...
    public bool HasParent() => (bool)Reference.Invoke("hasParent");
  }
}
