...
namespace Flowthru.Spark.ML.Feature
{
  ...
  /// </summary>
  /// <typeparam name="M"/>
  public abstract class JavaEstimator<M> : JavaPipelineStage, IEstimator<M>
    where M : JavaModel<M>
  {
    internal JavaEstimator(string className)
      : base(className) { }

    internal JavaEstimator(string className, string uid)
      : base(className, uid) { }

    internal JavaEstimator(JvmObjectReference jvmObject)
      : base(jvmObject) { }
