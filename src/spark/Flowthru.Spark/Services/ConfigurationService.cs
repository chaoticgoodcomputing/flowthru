...
namespace Flowthru.Spark.Services
{
  ...
  internal sealed class ConfigurationService : IConfigurationService
  {
    ...
    public const string WorkerWriteBufferSizeEnvVarName = "spark.dotnet.worker.writeBufferSize";
    ...
    private readonly ILoggerService _logger = LoggerServiceFactory.GetLogger(
    ...
    {
      if (!int.TryParse(GetEnvironmentVariable(DotnetBackendPortEnvVarName), out int portNumber))
      {
    ...
    {
      if (
        !int.TryParse(GetEnvironmentVariable(DotnetNumBackendThreadsEnvVarName), out int numThreads)
      )
      {
