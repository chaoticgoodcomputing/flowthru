...
namespace Flowthru.Spark.Network
{
  ...
  internal sealed class DefaultSocketWrapper : ISocketWrapper
  {
    ...
    public Stream InputStream =>
      _inputStream ??= CreateStream(ConfigurationService.WorkerReadBufferSizeEnvVarName);
    ...
    public Stream OutputStream =>
      _outputStream ??= CreateStream(ConfigurationService.WorkerWriteBufferSizeEnvVarName);
    ...
    private Stream CreateStream(string bufferSizeEnvVarName)
    {
      ...
      if (string.IsNullOrEmpty(envVar) || !int.TryParse(envVar, out var writeBufferSize))
      {
