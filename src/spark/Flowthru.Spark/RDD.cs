  ...
  internal class RDD<T> : IJvmObjectReferenceProvider
  {
    ...
    public RDD<T> Sample(bool withReplacement, double fraction, long? seed = null)
    {
      return new RDD<T>(
        (seed.HasValue)
          ? (JvmObjectReference)
            GetJvmRef().Invoke("sample", withReplacement, fraction, seed.GetValueOrDefault())
          : (JvmObjectReference)GetJvmRef().Invoke("sample", withReplacement, fraction),
        _sparkContext,
        _serializedMode
      );
    }
    ...
    private (int, string) CollectAndServe()
    {
      ...
      var pair = (JvmObjectReference[])
        rddRef.Jvm.CallStaticJavaMethod(
    ...
    JvmObjectReference IJvmObjectReferenceProvider.Reference
    {
      get
      {
        if (_jvmObject == null)
        {
          ...
          var pythonRdd = (JvmObjectReference)
