...
namespace Flowthru.Spark.Utils
{
  ...
  internal static class CommandSerDe
  {
    ...
    )
    {
      ...
      if (
        (udfData.MethodName != UdfWrapperMethodName)
    ...
    )
    {
      ...
      {
        udf = (DataFrameGroupedMapWorkerFunction.ExecuteDelegate)
          DeserializeUdfs<DataFrameGroupedMapWorkerFunction.ExecuteDelegate>(
            udfWrapperData,
            ref nodeIndex,
            ref udfIndex
          );
        ...
        nodeType == typeof(DataFrameWorkerFunction)
        || nodeType.IsSubclassOf(typeof(DataFrameUdfWrapper))
      )
      {
        udf = (DataFrameWorkerFunction.ExecuteDelegate)
          DeserializeUdfs<DataFrameWorkerFunction.ExecuteDelegate>(
            udfWrapperData,
            ref nodeIndex,
            ref udfIndex
          );
      ...
      {
        udf = (ArrowGroupedMapWorkerFunction.ExecuteDelegate)
          DeserializeUdfs<ArrowGroupedMapWorkerFunction.ExecuteDelegate>(
            udfWrapperData,
            ref nodeIndex,
            ref udfIndex
          );
    ...
    private static Delegate CreateUdfWrapperDelegate<T>(Type type, object[] parameters)
    {
      ...
      object udfWrapper = Activator.CreateInstance(type, bindingFlags, null, parameters, null);

      return Delegate.CreateDelegate(typeof(T), udfWrapper, UdfWrapperMethodName);
    }
  }
}
