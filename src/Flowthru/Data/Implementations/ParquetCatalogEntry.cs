...
public class ParquetCatalogEntry<T> : CatalogEntryBase<T>
    where T : System.Collections.IEnumerable {  // Parquet requires collections
  ...
  public override IO<T> Load() => IO.liftAsync(async () => {
    ...
    if (deserializeMethod == null) {
      throw new InvalidOperationException("Could not find ParquetSerializer.DeserializeAsync method");
    }

    await using var stream = File.OpenRead(_filePath);
    var task = (Task?)deserializeMethod.Invoke(null, new object?[] { stream, 0, null, CancellationToken.None });
    if (task == null) {
  ...
  public override IO<Unit> Save(T data) => IO.liftAsync(async () => {
    ...
    if (serializeMethod == null) {
      throw new InvalidOperationException("Could not find ParquetSerializer.SerializeAsync method");
    }

    await using var stream = File.Create(_filePath);
    await (Task)serializeMethod.Invoke(null, new object?[] { data, stream, null, CancellationToken.None })!;
    return unit;
  });
