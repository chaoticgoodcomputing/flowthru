...
/// </para>
/// </remarks>
public class ExcelCatalogEntry<T>
  : CatalogEntryBase<IEnumerable<T>>,
    ...
    return IO.liftAsync(async () =>
    {
      if (!File.Exists(_filePath))
      {
        throw new FileNotFoundException(
          $"Excel file not found for catalog entry '{Key}'",
          _filePath
        );
      }
  ...
  public override IO<ValidationResult> InspectShallow(int sampleSize = 100)
  {
    return IO.liftAsync(async () =>
    {
      try
      {
          ...
          return new ValidationResult(
            new[]
            {
              new ValidationError(
                ...
                $"Excel file not found: {_filePath}"
              ),
            }
          );
        }
        ...
        if (table == null)
        {
          var availableSheets = dataSet.Tables.Cast<DataTable>().Select(t => t.TableName).ToList();
