...
**RPC**: The protocol the [[Inspector]] exposes for other Tools — JSON-RPC 2.0 over stdio, frontend-launched per consumer; see [ADR-0012](/.claude/docs/adr/0012-inspector-rpc-protocol-and-surface.md). Carries read endpoints (DAG projection, manifest snapshot, Catalog Item metadata + preview) and dispatch endpoints (dry-run refresh, run-slice, cancel) plus log/step/end streaming notifications. Dispatch is scoped via the [[Frontend trust boundary]] — not by any protocol gate: dry-run is unscoped; real-run dispatch is the frontend's responsibility to gate (button click for Editor Frontends, agent elicitation for Agent Frontends). Contract defined in C# via `[RpcMethod]`-decorated interfaces; typed TypeScript client bindings are source-generated from the C# source.
...
**Frontend trust boundary**: The design principle that the [[Inspector]] trusts its caller frontend to have obtained any necessary user consent before dispatching real runs. The trust boundary lives at the frontend's user-interaction surface, not in the protocol — the Inspector is a local process and cannot meaningfully validate any consent payload (any field a frontend can set, a frontend can fabricate). Concrete consequences: Agent Frontends MUST elicit user consent via the agent host's elicitation primitive before calling `flowthru/run/start`; Editor Frontends rely on structural UI consent (no path to dispatch without a button click). The Inspector logs the caller identity from `flowthru/initialize` with every dispatch — the audit trail is the substantive safety story, not a runtime consent gate. See [ADR-0012](/.claude/docs/adr/0012-inspector-rpc-protocol-and-surface.md).
