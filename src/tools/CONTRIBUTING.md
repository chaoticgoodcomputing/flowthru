...
See [/CONTRIBUTING.md](/CONTRIBUTING.md) for cross-cutting design rules (the three error phases, decision rules for where validation belongs). Why Tools exist as a context distinct from Extensions is recorded under [`docs/adr/`](/docs/adr).
...
**RPC**: The protocol the [[Inspector]] exposes for other Tools — JSON-RPC 2.0 over stdio, frontend-launched per consumer; the protocol decision is recorded under [`src/tools/docs/adr/`](/src/tools/docs/adr). Carries read endpoints (DAG projection, manifest snapshot, Catalog Item metadata + preview) and dispatch endpoints (dry-run refresh, run-slice, cancel) plus log/step/end streaming notifications. Dispatch is scoped via the [[Frontend trust boundary]] — not by any protocol gate: dry-run is unscoped; real-run dispatch is the frontend's responsibility to gate (button click for Editor Frontends, agent elicitation for Agent Frontends). Contract defined in C# via `[RpcMethod]`-decorated interfaces; typed TypeScript client bindings are source-generated from the C# source.
...
**Frontend trust boundary**: The design principle that the [[Inspector]] trusts its caller frontend to have obtained any necessary user consent before dispatching real runs. The trust boundary lives at the frontend's user-interaction surface, not in the protocol — the Inspector is a local process and cannot meaningfully validate any consent payload (any field a frontend can set, a frontend can fabricate). Concrete consequences: Agent Frontends MUST elicit user consent via the agent host's elicitation primitive before calling `flowthru/run/start`; Editor Frontends rely on structural UI consent (no path to dispatch without a button click). The Inspector logs the caller identity from `flowthru/initialize` with every dispatch — the audit trail is the substantive safety story, not a runtime consent gate. See [`src/tools/docs/adr/`](/src/tools/docs/adr).
_Avoid_: "authentication boundary" (no credential exchange happens), "permission model" (no permissions are checked or enforced in the protocol), "consent gate" (the Inspector doesn't gate — it logs).

**Manifest snapshot**: A JSON file at `.flowthru/manifests/<flow-label>.json` carrying the `DagMetadataProjection` of a Flow Dev's project as it stood at the most recent successful pre-flight checkpoint — the "what built last" half of the F6 (built-last vs. now) diff. Emitted by `JsonMetadataProvider` during a Flowthru build or via the [[Inspector]]'s `flowthru/dryrun` RPC method (the manual refresh path); never refreshed on file save or editor idle, which would conflate "built-last" with "saved-last." Read by the Inspector's `flowthru/dag/snapshot` endpoint and surfaced to Editor Frontends. File mtime serves as the v1 freshness signal; richer meta (git SHA, project-state hash) is deferred until F6 surfaces demand. See [`src/tools/docs/adr/`](/src/tools/docs/adr).
_Avoid_: "DAG snapshot" (ambiguous — could mean a live in-memory DAG), "build artifact" (the snapshot is *metadata about* the build, not a deliverable; lives in `.flowthru/`, not `dist/`).
