...
namespace Flowthru.Core.Graph.Validation;
...
public class ValidationOptions
{
  ...
  /// </para>
  /// </remarks>
  internal InspectionLevel GetEffectiveInspectionLevel(INode node)
  {
    if (node == null)
    {
    ...
    var configuredLevel = GetInspectionLevel(node.Label);
    if (configuredLevel.HasValue)
    {
      return configuredLevel.Value;
    }

    // 2. For IItem nodes, check catalog-level preference (medium priority)
    if (node is IItem item && item.PreferredInspectionLevel.HasValue)
    {
      return item.PreferredInspectionLevel.Value;
    }

    // 3. Default to Shallow for items, None for other node types
    return node is IItem ? InspectionLevel.Shallow : InspectionLevel.None;
  }
