...
/// <summary>
/// Code fix for FU100: when a <c>[StepTest]</c> references a step whose service
/// dependency has no registered stub, this fix inserts a registration template into
/// an existing <c>[FUnitStubContainer]</c> in the project. If no container exists,
...
{
  private const string FUnitStubContainerAttributeFullName =
  ...
  public override async Task RegisterCodeFixesAsync(CodeFixContext context)
  {
    ...
    // Recover the service type's full name from diagnostic properties.
    if (
  ...
  )
  {
    ...
    var stub =
      ...
      + $"{indent}/// </summary>\n"
      + $"{indent}[global::Flowthru.FUnit.FUnitStubContainer]\n"
