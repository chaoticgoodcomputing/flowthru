...
contexts:
  - /src/core
exemplars:
  - /src/core/Flowthru.Core/Data/Storage/ByteLocation.cs
  - /src/core/Flowthru.Core/Data/Storage/SecretText.cs
  - /src/core/Flowthru.Core/Data/Storage/RemoteAccess.cs
---
...
- A `[System.Text.Json.Serialization.JsonConverter]` that **throws on write** — serialising a handoff violates [ADR-0020](/docs/adr/0020-s3-storage-medium-via-gateway-seam.md)'s catalog/DAG invariant; failing fast beats emitting `{}`.
- Equality via `CryptographicOperations.FixedTimeEquals`.

**Honestly-scoped limits, documented on the type** (per [ADR-0008](/docs/adr/0008-documentation-honesty-three-error-phases.md)):
