...
public class StepTestRegistryGenerator : IIncrementalGenerator
{
  ...
  private enum TestFramework { None, NUnit, XUnit, MSTest }
  ...
  public void Initialize(IncrementalGeneratorInitializationContext context)
  {
      ...
      .SyntaxProvider.CreateSyntaxProvider(
        ...
        transform: static (ctx, _) =>
        {
          ...
          var hasAttr = symbol
            .GetAttributes()
            .Any(a =>
              a.AttributeClass?.ToDisplayString() == FlowthruStepAttributeFullName
            );
        ...
        transform: static (ctx, _) =>
        {
          ...
          foreach (var attr in symbol.GetAttributes())
          {
            ...
            )
            {
              ...
              var baseRef = ns.Length > 0 && fqn.StartsWith(ns + ".")
                ? fqn.Substring(ns.Length + 1)
                : fqn;
    ...
    var framework = context.CompilationProvider.Select(static (compilation, _) =>
    {
      var assemblyNames = compilation.ReferencedAssemblyNames;
      if (assemblyNames.Any(static a => string.Equals(a.Name, "nunit.framework", System.StringComparison.OrdinalIgnoreCase)))
        return TestFramework.NUnit;
      if (assemblyNames.Any(static a => string.Equals(a.Name, "xunit.core", System.StringComparison.OrdinalIgnoreCase))
          || assemblyNames.Any(static a => string.Equals(a.Name, "xunit.v3.core", System.StringComparison.OrdinalIgnoreCase)))
        return TestFramework.XUnit;
      if (assemblyNames.Any(static a => string.Equals(a.Name, "Microsoft.VisualStudio.TestPlatform.TestFramework", System.StringComparison.OrdinalIgnoreCase)))
  ...
  )
  {
    ...
    foreach (var group in groups)
    {
      ...
      var runnerName = first.BaseClassRef.Replace(".", "_") + "_" + FrameworkRunnerSuffix(framework);
      var hasNamespace = first.TestClassNamespace.Length > 0;
      var indent = hasNamespace ? "    " : "";

      var sb = new StringBuilder();
      sb.AppendLine("// <auto-generated/>");
      sb.AppendLine("// FUnit test runner — generated by Flowthru.FUnit.SourceGenerators. Do not edit manually.");
  ...
  private static string FrameworkRunnerSuffix(TestFramework framework) => framework switch
  {
  ...
  private static string FrameworkUsing(TestFramework framework) => framework switch
  {
  ...
  private static string MethodAttribute(TestFramework framework) => framework switch
  {
