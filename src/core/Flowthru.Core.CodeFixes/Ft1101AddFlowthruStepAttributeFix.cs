...
/// <summary>
/// Code fix for FT1101: adds <c>[FlowthruStep]</c> to the step factory class referenced
...
/// </summary>
[
  ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(Ft1101AddFlowthruStepAttributeFix)),
  Shared
]
public sealed class Ft1101AddFlowthruStepAttributeFix : CodeFixProvider
{
  private const string FlowthruStepNamespace = "Flowthru.Step";
  ...
  public override async Task RegisterCodeFixesAsync(CodeFixContext context)
  {
    ...
    context.RegisterCodeFix(
      CodeAction.Create(
        ...
        equivalenceKey: nameof(Ft1101AddFlowthruStepAttributeFix)
      ),
      diagnostic
    );
  }
  ...
  )
  {
    ...
    // Ensure `using Flowthru.Step;` is present so the unqualified attribute resolves.
    newRoot = EnsureUsing(newRoot, FlowthruStepNamespace);
