...
**Hermetic pre-flight**: The introspection rung that performs *no I/O* — structure and wiring validated against in-process state alone (DAG soundness, DI-registration presence, dispatcher presence), with no socket, file, or database access. The organizing axis of `ValidationDepth` is this I/O boundary, not row depth: `Hermetic` below the line, `Shallow`/`Deep` above it. The use is an offline smoke test (`DryRun.On + Hermetic`). Distinct from the **plan-build precondition** — assembling and acyclicity-checking the merged DAG, which is required to produce *any* runnable plan and so happens at every depth, surfacing failures as `Validated` data rather than gating on depth. See [`docs/adr/`](/docs/adr).
