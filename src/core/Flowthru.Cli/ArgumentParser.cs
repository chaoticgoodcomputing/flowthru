...
/// <c>--from</c>/<c>--to</c>/<c>--only</c> — the two paths have
/// different slicing semantics. The parser rejects the combination at
/// the boundary.
/// </para>
/// </remarks>
...
public static class ArgumentParser
{
  ...
  public static CliArguments Parse(IReadOnlyList<string> args)
  {
    ...
    var slice = BuildSliceStrategy(fromLabels, toLabels, onlyLabels);
    if (flowLabel is not null && slice is not null)
    {
      throw new ArgumentException(
        "--flow cannot be combined with --from / --to / --only. Use one or the other."
      );
    }
  ...
  /// <summary>
  /// Build a slice strategy from the collected From/To/Only labels.
  /// Each primitive (when populated) becomes a <see cref="FlowSliceStrategy.From"/>
  /// / <see cref="FlowSliceStrategy.To"/> / <see cref="FlowSliceStrategy.Only"/>;
  /// when more than one primitive is populated they compose via
  /// <see cref="FlowSliceStrategy.And"/> (intersection). All-empty
  /// returns <c>null</c> — the caller treats that as "no slice".
  /// </summary>
  ...
  )
  {
    ...
    return primitives.Count switch
    {
      0 => null,
      1 => primitives[0],
  ...
  public const string HelpText =
    ...
    + "--flow cannot be combined with --from / --to / --only.\n";
}
