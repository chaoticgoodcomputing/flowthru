  ...
  where T : notnull, IFlatSchema
{
  ...
  private static readonly IReadOnlyDictionary<Type, IReadOnlyList<Type>> s_sparkToClrCompatibility =
  ...
  public IEnumerable<T> Collect(TypedFrame<T> frame)
  {
    ...
    if (errors.Count > 0)
    {
      ...
      throw new InvalidOperationException(
        $"DataFrame schema is incompatible with '{typeof(T).Name}':" + Environment.NewLine + details
      );
    }
  ...
  private ConstructorInfo? FindRecordConstructor()
  {
    ...
    return typeof(T)
      ...
      .Where(c =>
      {
        ...
        return ps.Length > 0 && ps.All(p => properties.Contains(p.Name ?? ""));
      })
  ...
  private static bool IsCompatible(DataType sparkType, Type clrType)
  {
    ...
    if (s_sparkToClrCompatibility.TryGetValue(sparkType.GetType(), out var compatibleClrTypes))
