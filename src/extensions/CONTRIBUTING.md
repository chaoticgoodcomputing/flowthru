...
Every extension ships an agent [[Skill shard]] at `src/extensions/<Extension>/skill/SKILL.md`. It sits beside the README and serves the same audience the README's "Mental model" section does — a Flow/Catalog Developer meeting this stack — but written *for an agent working in a downstream project*, where a `README.md` isn't discoverable but a skill is. (The same convention extends to core packages that are optional add-ons from a Flow Developer's perspective — e.g. `src/core/Flowthru.FUnit/skill/SKILL.md` — the tooling scans `src/{extensions,core}/*/skill/SKILL.md`. The umbrella `flowthru` skill itself lives at `src/core/Flowthru.Core/skill/`.)

**The `skill/` subdir is mandatory, not cosmetic.** `npx skills add` copies the *entire directory* that contains the `SKILL.md` — its only exclusions are `metadata.json`, `.git`, and Python caches. A shard placed directly at `src/extensions/<Ext>/SKILL.md` would therefore drag the whole extension's `.cs` source and `.csproj` into the user's skills directory. Isolating each skill in its own `skill/` subdir (holding only skill markdown) is what keeps an install clean.

The shard is deliberately **not** `SKILL.md`'s default discovery target: it lives under `src/`, which no agent harness walks, so it never pollutes an in-repo session. It becomes discoverable two ways, both driven by the repo-root `.claude-plugin/marketplace.json` manifest that declares each `.../skill` dir:
...
- Honor the three error phases ([[Design-time error|design-time]], [[Pre-flight error|pre-flight]], [[Runtime error (phase)|runtime]]) — push validation as early as possible
...
**Skill shard**: The agent-facing `SKILL.md` an extension ships at `src/extensions/<Extension>/skill/SKILL.md` — the companion to its README, written for an agent working in a downstream project. The `skill/` subdirectory is load-bearing, not cosmetic: `npx skills add` copies the whole directory containing a `SKILL.md`, so a shard placed directly in the package directory would drag the extension's `.cs` and `.csproj` files into a user's skills directory. Its `metadata.flowthru` frontmatter (`extension`, `surface`, `capability`, `register`) is generated into the umbrella `flowthru` skill's `extensions.md` capability index; its body is a standalone deep skill installable via `npx skills add … --skill flowthru-<ext>`. See [The Skill Shard](#the-skill-shard).
...
**Wide vs narrow transform (Extension Developer)**: Spark's dependency vocabulary, adopted to classify step logic by whether it can stream. A *narrow* transform (map, filter) emits each output row from bounded per-row context, so it runs in O(batch) memory over a streaming source; a *wide* transform (global sort, dedup, aggregate, join) must consume its entire input before emitting any output, so it can never stream — it either materializes the eager view or delegates to an engine that executes it outside the CLR. Flowthru is single-node, so the operative property here is "cannot stream," not Spark's "requires a shuffle." The Flow Developer entry, [[Wide vs narrow transform (Flow Developer)]], classifies the same split by what a Step *author* can see — all of its input before emitting — rather than by streamability; the two agree on which transforms are wide.
