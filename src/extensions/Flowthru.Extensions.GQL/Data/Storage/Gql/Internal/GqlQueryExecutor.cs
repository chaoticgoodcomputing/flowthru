...
namespace Flowthru.Data.Storage.Gql.Internal;

/// <summary>
/// Pagination execution helpers shared by <see cref="GqlQuery{TResult,T}"/>
/// and <see cref="GqlQuery{TFilter,TResult,T}"/>. Internal — adapter
/// authors use the public smart constructors, not the executor directly.
/// </summary>
internal static class GqlQueryExecutor
{
  ...
  internal static async Task<List<T>> NonPagedAsync<TResult, T>(
    ...
    where T : class
  {
    var result = await queryFunc(ct).ConfigureAwait(false);
    ...
    where T : class
  {
    ...
    while (true)
    {
      ...
      try
      {
        result = await queryFunc(cursor, pageSize, ct).ConfigureAwait(false);
      }
      catch (Exception ex)
      {
        throw new InvalidOperationException(
    ...
    where T : class
  {
    ...
    while (true)
    {
      ...
      try
      {
        result = await queryFunc(offset, pageSize, ct).ConfigureAwait(false);
      }
      catch (Exception ex)
      {
        throw new InvalidOperationException(
  ...
  internal static async Task<List<T>> FilteredNonPagedAsync<TFilter, TResult, T>(
    ...
    where T : class
  {
    var result = await queryFunc(filter, ct).ConfigureAwait(false);
    result.EnsureNoErrors();
    ...
    where T : class
  {
    ...
    while (true)
    {
      ...
      try
      {
        result = await queryFunc(filter, cursor, pageSize, ct).ConfigureAwait(false);
      }
      catch (Exception ex)
      {
        throw new InvalidOperationException(
    ...
    where T : class
  {
    ...
    while (true)
    {
      ...
      try
      {
        result = await queryFunc(filter, offset, pageSize, ct).ConfigureAwait(false);
      }
      catch (Exception ex)
      {
        throw new InvalidOperationException(
  ...
  // ── Probe helpers (used by GqlQueryStorageAdapter.InspectShallow) ────
    ...
    where T : class
  {
    try
    {
      // Use page size 1 for paginated, or run non-paged as-is for a probe.
      GqlQuery<TResult, T> probeQuery =
        ...
        : query.OffsetQueryFunc is not null
          ? new(query.Label, query.OffsetQueryFunc, query.OffsetPagination!, 1, allowEmptyData: true)
        : query;

      await probeQuery.ToListAsync(ct).ConfigureAwait(false);
      return true;
    }
    catch
    {
  ...
  internal static async Task<bool> FilteredProbeAsync<TFilter, TResult, T>(
    ...
    where T : class
  {
    // Probe with a null filter — validates connectivity without
    // requiring the caller to supply a filter value.
    try
    {
      await query.WithFilter(null!).ToListAsync(ct).ConfigureAwait(false);
      return true;
    }
    catch
    {
