...
/// <summary>
/// Stub StrawberryShake <see cref="IOperationResult{T}"/> used in unit
/// tests — no real client wiring required, just enough to drive
/// success/error paths through the adapter shapes.
/// </summary>
public sealed class StubOperationResult<T> : IOperationResult<T>
  where T : class
{
  ...
  public static StubOperationResult<T> EmptyData() => new() { Data = null };
}

public sealed class StubClientError : IClientError
{
...
internal sealed class NullDataFactory : IOperationResultDataFactory
{
...
internal sealed class NullDataFactory<T> : IOperationResultDataFactory<T>
  where T : class
{
