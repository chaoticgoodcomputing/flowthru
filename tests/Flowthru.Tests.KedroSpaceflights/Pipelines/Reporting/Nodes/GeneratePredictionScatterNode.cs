...
/// </para>
/// </remarks>
public class GeneratePredictionScatterNode
    : NodeBase<(IEnumerable<ModelMetrics> Metrics, IEnumerable<ModelPredictions> Predictions), GenericChart> {
  protected override Task<GenericChart> Transform(
      (IEnumerable<ModelMetrics> Metrics, IEnumerable<ModelPredictions> Predictions) input) {
    ...
    var overEstimateTrace = CSharpChart.Point<double, double, string>(
    ...
    .WithMarkerStyle(
      Color: Color.fromKeyword(ColorKeyword.Orange),
      Size: 8,
      Opacity: 0.25
    );

    var underEstimateTrace = CSharpChart.Point<double, double, string>(
    ...
    .WithMarkerStyle(
      Color: Color.fromKeyword(ColorKeyword.Red),
      Size: 8,
      Opacity: 0.25
    );

    // Create 1:1 identity reference line (dotted) in log space
    var referenceLine = CSharpChart.Line<double, double, string>(
    ...
    // Combine all traces
    var chart = Plotly.NET.Chart.Combine(new[] { underEstimateTrace, overEstimateTrace, referenceLine })
        .WithTitle($"OLS Model Results (R² = {metrics.R2Score:F2})")
        .WithSize(600, 600);

    // Configure axes with log scale, title, and no grid
    chart = chart.WithXAxis(LinearAxis.init<IConvertible, IConvertible, IConvertible, IConvertible, IConvertible, IConvertible, IConvertible, IConvertible>(
        Title: Title.init("Actual Price (ln($))"),
        ShowGrid: false
    ));

    chart = chart.WithYAxis(LinearAxis.init<IConvertible, IConvertible, IConvertible, IConvertible, IConvertible, IConvertible, IConvertible, IConvertible>(
        Title: Title.init("Predicted Price (ln($))"),
        ShowGrid: false
    ));
