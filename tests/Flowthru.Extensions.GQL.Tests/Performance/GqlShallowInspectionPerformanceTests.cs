...
public class GqlShallowInspectionPerformanceTests
{
    ...
    public async Task NonPaginated_ShallowInspect_CompletesWithinBudget_On10kItems()
    {
        ...
        Assert.That(callCount, Is.EqualTo(1), "Expected exactly one query call during shallow inspection");

        TestContext.Out.WriteLine($"GQL non-paginated shallow inspection ({LargeDatasetSize} items, sample={SampleSize}): {sw.ElapsedMilliseconds}ms, calls={callCount}");
    }
    ...
    public async Task RelayPaginated_ShallowInspect_FetchesOnlyOnePage()
    {
        ...
        Task<IOperationResult<TestPagedResult>> RelayQueryFunc(string? cursor, int pageSize, CancellationToken ct)
        {
            Interlocked.Increment(ref callCount);
            var nodes = Enumerable.Range(1, pageSize)
                .Select(i => new TestUser { Id = i, Name = $"User-{i}" })
                .ToList();
        ...
        var adapter = new GqlEnumerableStorageAdapter<TestPagedResult, TestUser>(
            ...
            pagination: Pagination.Relay<TestPagedResult, TestUser>(
                getNodes: r => r.Nodes,
                getPageInfo: r => r.PageInfo is { } pi
        ...
        Assert.That(result.IsValid, Is.True, string.Join(", ", result.Errors.Select(e => e.Message)));
        Assert.That(callCount, Is.EqualTo(1), "Relay paginated shallow inspection should make exactly one page request");

        TestContext.Out.WriteLine($"GQL relay paginated shallow inspection: {sw.ElapsedMilliseconds}ms, calls={callCount}");
    }
    ...
    public async Task OffsetPaginated_ShallowInspect_FetchesOnlyOnePage()
    {
        ...
        Task<IOperationResult<TestPagedResult>> OffsetQueryFunc(int offset, int limit, CancellationToken ct)
        {
            Interlocked.Increment(ref callCount);
            var nodes = Enumerable.Range(offset + 1, limit)
                .Select(i => new TestUser { Id = i, Name = $"User-{i}" })
                .ToList();
            var data = new TestPagedResult
            {
                Nodes = nodes,
                Total = LargeDatasetSize  // tell adapter there are many more rows
            };
            return Task.FromResult<IOperationResult<TestPagedResult>>(
                StubOperationResult<TestPagedResult>.Success(data)
            );
        ...
        Assert.That(result.IsValid, Is.True, string.Join(", ", result.Errors.Select(e => e.Message)));
        Assert.That(callCount, Is.EqualTo(1), "Offset paginated shallow inspection should make exactly one page request");

        TestContext.Out.WriteLine($"GQL offset paginated shallow inspection: {sw.ElapsedMilliseconds}ms, calls={callCount}");
    }
}
