...
namespace Flowthru.Tests.Spark
{
  ...
  public class CallbackTests
  {
    ...
    public async Task TestCallbackIds()
    {
      ...
      {
        tasks.Add(Task.Run(() => ids.Add(callbackServer.RegisterCallback(callbackHandler))));
      }
    ...
    public void TestCallbackHandlers()
    {
      ...
      int inputToHandler = 1;
      {
        ...
        var callbackHandler = new TestCallbackHandler { Id = 1 };
        ...
        Assert.Equal(callbackHandler.Apply(inputToHandler), callbackHandler.Inputs.First());
      }
      {
        // Test CallbackConnection using a ICallbackHandler that
        // throws an exception.
        var callbackHandler = new ThrowsExceptionHandler { Id = 2 };
        callbackHandlersDict[callbackHandler.Id] = callbackHandler;
        TestCallbackConnection(
          callbackHandlersDict,
          callbackHandler,
      ...
      }
      {
        ...
        tokenSource.Cancel();
        var callbackHandler = new TestCallbackHandler { Id = 3 };
        callbackHandlersDict[callbackHandler.Id] = callbackHandler;
        TestCallbackConnection(
          callbackHandlersDict,
          callbackHandler,
