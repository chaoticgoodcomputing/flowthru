  ...
  public class TypeConverterTests
  {
    ...
    public void TestArrayArray()
    {
      int[][] expected = Enumerable
        .Range(0, 10)
        .Select(i => Enumerable.Range(i, 10).ToArray())
        .ToArray();
    ...
    {
      var expected = new Hashtable(Enumerable.Range(0, 10).ToDictionary(k => k, v => v * v));
    ...
    {
      Dictionary<int, int> expected = Enumerable.Range(0, 10).ToDictionary(i => i, i => i * i);
    ...
    public void TestDictionaryDictionary()
    {
      Dictionary<int, Dictionary<int, int>> expected = Enumerable
        .Range(0, 10)
        .ToDictionary(i => i, i => Enumerable.Range(i, 10).ToDictionary(j => j, j => j * j));
      ...
      Dictionary<int, Dictionary<int, int>> actual = TypeConverter.ConvertTo<
    ...
    {
      {
        Dictionary<int, int[]> expected = Enumerable
          .Range(0, 10)
          .ToDictionary(i => i, i => Enumerable.Range(i, 10).ToArray());
      ...
      }
      {
        ...
        Dictionary<int, int>[] actual = TypeConverter.ConvertTo<Dictionary<int, int>[]>(arrayList);
        Assert.Equal(expected, actual);
      }
    }
  }
