  ...
  public class WorkerFunctionTests
  {
    ...
    public void TestPicklingWorkerFunction()
    {
      var func = new PicklingWorkerFunction(
        new PicklingUdfWrapper<string, string>((str) => str).Execute
      );
    ...
    public void TestChainingPicklingWorkerFunction()
    {
      var func1 = new PicklingWorkerFunction(
        new PicklingUdfWrapper<int, string, string>((number, str) => $"{str}:{number}").Execute
      );

      var func2 = new PicklingWorkerFunction(
        new PicklingUdfWrapper<string, string>((str) => $"outer1:{str}").Execute
      );

      var func3 = new PicklingWorkerFunction(
        new PicklingUdfWrapper<string, string>((str) => $"outer2:{str}").Execute
      );
    ...
    public void TestArrowWorkerFunction()
    {
      var func = new ArrowWorkerFunction(
        new ArrowUdfWrapper<StringArray, StringArray>((str) => str).Execute
      );

      string[] input = { "arg1" };
      ArrowTestUtils.AssertEquals(input[0], func.Func(new[] { ToArrowArray(input) }, new[] { 0 }));
    }
    ...
    public void TestDataFrameWorkerFunctionForBool()
    {
      ...
      var stringColumn = (StringArray)ToArrowArray(
        new[] { "arg1_true", "arg1_true", "arg1_false", "arg1_false" }
      );

      ArrowStringDataFrameColumn ArrowStringDataFrameColumn = ToArrowStringDataFrameColumn(
        stringColumn
      );
      var boolColumn = new BooleanDataFrameColumn(
        "Bool",
        Enumerable.Range(0, 4).Select(x => x % 2 == 0)
      );
      var input = new DataFrameColumn[] { ArrowStringDataFrameColumn, boolColumn };
    ...
    public void TestChainingArrowWorkerFunction()
    {
      ...
      var input = new IArrowArray[] { ToArrowArray(new[] { 100 }), ToArrowArray(new[] { "name" }) };

      // Validate one-level chaining.
      ArrowWorkerFunction chainedFunc1 = ArrowWorkerFunction.Chain(func1, func2);
      AssertEquals("outer1:name:100", chainedFunc1.Func(input, new[] { 0, 1 }));

      // Validate two-level chaining.
      ArrowWorkerFunction chainedFunc2 = ArrowWorkerFunction.Chain(chainedFunc1, func3);
      AssertEquals("outer2:outer1:name:100", chainedFunc2.Func(input, new[] { 0, 1 }));
    }
    ...
    public void TestChainingDataFrameWorkerFunction()
    {
      ...
      ArrowTestUtils.AssertEquals("outer1:name:100", chainedFunc1.Func(input, new[] { 0, 1 }));
    ...
    public void TestInvalidChainingArrow()
    {
      ...
      IArrowArray[] input = new[] { ToArrowArray(new[] { 100 }), ToArrowArray(new[] { "name" }) };
