...
[Category("Python")]
[Category("Validation")]
public class PythonStepValidationTests
{
...
#pragma warning restore NUnit1032
  ...
  public void ValidateRegistration_MissingFunction_ThrowsException()
  {
    ...
    var ex = Assert.Throws<InvalidOperationException>(
      () =>
        new PythonStepWrapper<ModelConfigSchema, ModelResultSchema>(
          _executor,
          "_Fixtures.validation_test_steps",
  ...
  [Test]
  public void ValidateRegistration_ValidStep_Succeeds()
  {
    // Arrange & Act
  ...
  {
    // Arrange
    var pipeline = CreateTestFlow("_Fixtures.validation_test_steps", "wrong_input_schema");
  ...
  {
    // Arrange
    var pipeline = CreateTestFlow("_Fixtures.validation_test_steps", "wrong_output_schema");
  ...
  {
    // Arrange
    var pipeline = CreateTestFlow("_Fixtures.validation_test_steps", "too_many_inputs");
  ...
  {
    // Arrange
    var pipeline = CreateTestFlow("_Fixtures.validation_test_steps", "too_many_outputs");
  ...
  {
    // Arrange
    var pipeline = CreateTestFlow("_Fixtures.validation_test_steps", "zero_inputs");
  ...
  {
    // Arrange
    var pipeline = CreateTestFlow("_Fixtures.validation_test_steps", "zero_outputs");
  ...
  {
    // Arrange
    var pipeline = CreateTestFlow("_Fixtures.validation_test_steps", "valid_step");
  ...
  /// </summary>
  private Flow CreateTestFlow(string moduleName, string functionName)
  {
    var pipeline = new Flow();
    ...
    var addStepMethod = typeof(Flow).GetMethod(
      "AddStep",
      System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance
    );

    if (addStepMethod == null)
    {
      throw new InvalidOperationException("Could not find Flow.AddStep method via reflection");
    }

    addStepMethod.Invoke(pipeline, new object[] { pipelineStep });
