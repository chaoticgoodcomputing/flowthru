...
└── <FlowName>/
    ├── <FlowName>Pipeline.cs   # Steps wired to input/output catalog entries
    └── Steps/
...
5. **Write nodes.** Implement the transformation logic in `Pipelines/<FlowName>/Steps/`. Each node declares typed inputs and outputs matching the schemas from step 3.
