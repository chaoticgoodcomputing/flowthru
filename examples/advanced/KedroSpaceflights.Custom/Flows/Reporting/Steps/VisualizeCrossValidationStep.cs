...
namespace KedroSpaceflights.Custom.Flows.Reporting.Steps;
...
public static class VisualizeCrossValidationStep
{
  public static Func<CrossValidationResults, Task<GenericChart>> Create(ILogger? logger = null)
  {
    return async (input) =>
    {
      ...
      var scatterY = r2Scores
        .Select(r2 => NormalProbabilityDensity(r2, results.MeanR2Score, results.StdDevR2Score))
        .ToList();
      ...
      var scatterTrace = CSharpChart
        .Point<double, double, double>(scatterX, scatterY)
      ...
      var chart = Plotly
        .NET.Chart.Combine(new[] { curveTrace, scatterTrace, meanLineTrace, kedroLineTrace })
  ...
  {
    return CSharpChart
      .Line<double, double, double>(new[] { xValue, xValue }, new[] { yMin, yMax })
      .WithTraceInfo(Name: name, ShowLegend: true)
      .WithLineStyle(
        Color: Color.fromKeyword(color),
        Width: 2.0,
