  ...
  > Create()
  {
    return (input) =>
    {
      ...
      var predictionPairs = actuals
        .Zip(
          predictions,
          (actual, predicted) => new ModelPredictions
          {
            // Wrap into the layer-local NewTypes generated by [FlowthruColumn].
            // Confusing actual with predicted is now a compile-time error in downstream steps.
            Actual = new ActualReviewScore(actual),
            Predicted = new PredictedReviewScore(predicted),
          }
        )
        .ToList();
