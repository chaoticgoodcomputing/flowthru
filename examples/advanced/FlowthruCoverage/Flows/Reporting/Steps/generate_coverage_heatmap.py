...
def generate_coverage_heatmap(pivot_coverage: pd.DataFrame) -> bytes:
    ...
    src_subgroup_of = dict(zip(pivot_coverage["SrcPackage"], pivot_coverage["SrcSubgroup"]))
    ...
    for src_pkg in pivot.index:
        ...
        for test_proj in pivot.columns:
            ...
            if src_pkg in ghost_src_set:
                row_hover.append(
                    f"<b>{src_pkg}</b><br>"
                    f"<i>Not found in any coverage report</i>"
                )
    ...
    fig = go.Figure(
        data=go.Heatmap(
            ...
            zmin=-1,
            zmax=100,
            colorbar=dict(title="Coverage %", ticksuffix="%", tickvals=[0, 25, 50, 75, 100]),
        )
    )
    ...
    def _flush_section(section, start, end):
        ...
        annotations.append(dict(
            x=mid, y=1.10, xref="x", yref="paper",
            text=f"<b>{section}</b>",
