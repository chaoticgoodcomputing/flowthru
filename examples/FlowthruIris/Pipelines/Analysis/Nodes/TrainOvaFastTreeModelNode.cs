...
/// </para>
/// </summary>
public class TrainOvaPerceptronModelNode : NodeBase<IEnumerable<IrisSchema>, (ITransformer, MulticlassClassificationMetrics)> {
  protected override Task<(ITransformer, MulticlassClassificationMetrics)> Transform(IEnumerable<IrisSchema> input) {
    ...
    // Train the model
    Logger?.LogInformation("Fitting OVA + Averaged Perceptron model to {Count} samples", input.Count());
