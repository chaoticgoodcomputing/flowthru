...
public class MLNetModelCatalogEntry : CatalogEntryBase<ModelWithSchema>
{
  ...
  public MLNetModelCatalogEntry(string key, string modelPath, MLContext? mlContext = null)
    : base(key)
  {
    ...
    return Aff(() =>
    {
      if (!File.Exists(_modelPath))
      {
        throw new FileNotFoundException(
          $"Model file not found for catalog entry '{Key}'",
          _modelPath
        );
      }
      ...
      catch (Exception ex) when (ex is not FileNotFoundException)
      {
        throw new InvalidOperationException(
          $"Failed to load ML.NET model for catalog entry '{Key}'",
          ex
        );
      }
    });
  ...
  public override Aff<Unit> Save(ITransformer model)
  {
    return Aff(() =>
    {
      ...
      {
        throw new InvalidOperationException(
          $"Failed to save ML.NET model for catalog entry '{Key}'",
          ex
        );
      }
    });
  ...
  public override Aff<ValidationResult> InspectShallow(int sampleSize = 100)
  {
    return Aff(() =>
    {
      ...
      return errors.Count > 0 ? new ValidationResult(errors) : new ValidationResult(); // Success
    });
  }
