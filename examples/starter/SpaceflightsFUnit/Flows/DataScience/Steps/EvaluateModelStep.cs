...
public static class EvaluateModelStep
{
  ...
  public class Tests : FUnitContext
  {
    private static LinearRegressionModel IdentityModel =>
      new()
      {
        // Intercept = 0, one coefficient = 1.0 mapped to first feature (engines)
        // For test isolation we build predictions directly rather than going through Predict.
        Intercept = 0,
        Coefficients = new double[] { 1.0, 0, 0, 0, 0, 0, 0 },
        FeatureNames = new[]
        {
          "engines",
          "passenger_capacity",
    ...
    private static TestData TestRow(decimal label) =>
      new()
      {
    ...
    public void EmptyInput_ReturnsZeroMetrics()
    {
      ...
      Assert.That(metrics.MeanAbsoluteError, Is.EqualTo(0m));
      Assert.That(metrics.MaxError, Is.EqualTo(0m));
