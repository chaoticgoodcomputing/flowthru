 ...
 *
 * Source of truth: `src/{extensions,core}/<Pkg>/skill/SKILL.md` frontmatter —
 *   metadata.flowthru.{extension, surface, capability, register}  (+ top-level `name`)
 * A skill dir whose SKILL.md has NO `metadata.flowthru` block is the umbrella
 * itself (`src/core/Flowthru.Core/skill`), not a shard — it is skipped here.
 * See src/extensions/CONTRIBUTING.md § "The Skill Shard" for the shard contract.
 *
 * Each skill lives in an ISOLATED `<Pkg>/skill/` subdir so `npx skills add`
 * (which copies the whole dir containing SKILL.md) never drags package source.
 *
 * Two generated artifacts (both fully derived — never hand-edit):
 *   1. `src/core/Flowthru.Core/skill/extensions.md` — the managed block between
...
const UMBRELLA_SKILL_DIR = './src/core/Flowthru.Core/skill';
const EXTENSIONS_MD = join(ROOT, 'src', 'core', 'Flowthru.Core', 'skill', 'extensions.md');
...
function collectShards() {
  ...
  }
  {
    for (const pkgDir of entries.sort((a, b) => a.localeCompare(b))) {
      ...
      if (!existsSync(skillPath)) continue; // not every package has a skill
      ...
      if (!ft) continue; // no metadata.flowthru → the umbrella itself, not a shard
      const missing = [];
      ...
      shards.push({
        dir: `./${relative(ROOT, skillDir).split('\\').join('/')}`,
        name: `${fm.name}`,
...
function renderManifest(shards) {
  const manifest = {
    '//': 'GENERATED by scripts/generate-skill-extensions.mjs from src/{extensions,core}/*/skill/SKILL.md — do not edit by hand.',
    plugins: [
      {
        name: 'flowthru',
